...
func (a ItemDetails) Len() int           { return len(a) }
func (a ItemDetails) Swap(i, j int)      { a[i], a[j] = a[j], a[i] }
func (a ItemDetails) Less(i, j int) bool { return a[i].CreatedAt < a[j].CreatedAt }
...
func getTransactionEvidencesByItemIDs(q sqlx.Queryer, itemIDs []int64) (transactionEvidences map[int64]TransactionEvidence, err error) {
	if len(itemIDs) == 0 {
		return transactionEvidences, err
	}

	transactionEvidenceTmp := []TransactionEvidence{}
	args := make([]interface{}, len(itemIDs))
	for i, id := range itemIDs {
		args[i] = id
	}
	query := `SELECT item_id, id, status FROM transaction_evidences WHERE item_id in (?` + strings.Repeat(",?", len(args)-1) + `)`
	err = sqlx.Select(q, &transactionEvidenceTmp, query, args...)

	transactionEvidences = make(map[int64]TransactionEvidence)
	for _, row := range transactionEvidenceTmp {
		transactionEvidencesRow := TransactionEvidence{}
		transactionEvidencesRow.ID = row.ID
		transactionEvidencesRow.ItemID = row.ItemID
		transactionEvidencesRow.Status = row.Status

		transactionEvidences[row.ItemID] = transactionEvidencesRow
	}
...
func getUserSimpleByIDs(q sqlx.Queryer, userIDs []int64) (userSimples map[int64][]UserSimple, err error) {
	...
	stmt := `SELECT * FROM users WHERE id in (?` + strings.Repeat(",?", len(args)-1) + `)`
	err = sqlx.Select(q, &users, stmt, args...)

	userSimple2 := make(map[int64][]UserSimple)
	for _, row := range users {
		userSimple := UserSimple{}
		userSimple.ID = row.ID
		userSimple.AccountName = row.AccountName
		userSimple.NumSellItems = row.NumSellItems

		userSimple2[row.ID] = append(userSimple2[row.ID], userSimple)
	}

	userSimples = userSimple2
	return userSimples, err
}
...
func getCategoryByIDs(q sqlx.Queryer, categoryIDs []int) (categories map[int][]Category, err error) {
	categoriesTemp := []Category{}
	args := make([]interface{}, len(categoryIDs))
	for i, id := range categoryIDs {
		args[i] = id
	}
	stmt := `SELECT * FROM categories WHERE id in (?` + strings.Repeat(",?", len(args)-1) + `)`
	err = sqlx.Select(q, &categoriesTemp, stmt, args...)

	categories2 := make(map[int][]Category)
	for _, row := range categoriesTemp {
		if row.ParentID != 0 {
			parentCategory, err := getCategoryByID(q, row.ParentID)
			if err != nil {
				// 	return category, err
			}
			row.ParentCategoryName = parentCategory.CategoryName
		}

		categories2[row.ID] = append(categories2[row.ID], row)
	}
...
func getNewItems(w http.ResponseWriter, r *http.Request) {
	...
	sellers, err := getUserSimpleByIDs(dbx, userIDs)
	if err != nil {
	...
	for _, item := range items {
		itemSimples = append(itemSimples, ItemSimple{
			...
			Seller:     &sellers[item.SellerID][0],
			Status:     item.Status,
			Name:       item.Name,
			Price:      item.Price,
...
func getNewCategoryItems(w http.ResponseWriter, r *http.Request) {
	...
	itemSimples := []ItemSimple{}
	var userIDs []int64
	var categoryIDss []int
	for _, item := range items {
		userIDs = append(userIDs, item.SellerID)
		categoryIDss = append(categoryIDss, item.CategoryID)
	}
	...
	categories, err := getCategoryByIDs(dbx, categoryIDss)
	if err != nil {
		outputErrorMsg(w, http.StatusNotFound, "category not found")
		return
	}
...
func getTransactions(w http.ResponseWriter, r *http.Request) {
	...
	// transactionEvidences, err := getTransactionEvidencesByItemIDs(tx, itemIDs)
	// if err != nil {
	// 	outputErrorMsg(w, http.StatusInternalServerError, "transaction evidences not found")
	// 	return
	// }
	...
	for _, item := range items {
		...
		if item.BuyerID != 0 {
			...
			itemDetail.Buyer = &buyers[item.BuyerID][0]
		}
		...
		if transactionEvidence.ID > 0 {
			// if transactionEvidences[item.ID].ID > 0 {
			shipping := Shipping{}
	...
	//wg.Wait()
	//sort.Sort(ItemDetails(itemDetails))
	//tx.Commit()
